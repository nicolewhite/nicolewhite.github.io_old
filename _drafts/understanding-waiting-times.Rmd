...
Let's say I've crunched the numbers and found that we receive an average of `r lambda` requests from this webhook per minute. Thus, the time-unit \\(t\\) is one minute and the rate \\(\lambda\\) is `r lambda`. Knowing these, we can answer questions such as:
...
The probability of waiting 1.5 minutes for the next request is `r round(100 * exp(-lambda*t), 2)`%.
